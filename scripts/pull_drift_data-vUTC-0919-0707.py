#!/usr/bin/env python3
# pull_drift_data.py
# Version: 1.1 (built with python-script-standard v3.10)
# Required Python: 3.8+
# Series: standalone
...
def _previous_ytd_range(existing):
    """The range this run should re-measure: the newest one written before today.
    ...
    prior = [r for r in existing
             if (r.get("row_type") == "ytd") and (r.get("cohort_date") or "")]
    if not prior:
        return None
...
def main():
    ...
    new_range = f"{close.year}-01-01:{close:%Y-%m-%d}"
    prev_range = _previous_ytd_range(kept)
    ...
    print()
    for rng, kind in targets:
        start, end = rng.split(":")
        row = dict(stamp, row_type="ytd", cohort_date=rng, close_date=end,
                   cohort_dow="", age_days="")
        ok, err = _measure(f"{start.replace('-', '/')}:{end.replace('-', '/')}"
                           f"[{DATE_FIELD}]", row)
        if ok:
            print(f"  YTD {rng}  {kind:<8} all_sb {row['all_sb']:>9,}  "
                  f"ft {row['fulltext']:>9,}")
        else:
            failed.append(rng)
            print(f"  YTD {rng}  {kind:<8} FAILED -- {err}")
        fresh.append(row)
