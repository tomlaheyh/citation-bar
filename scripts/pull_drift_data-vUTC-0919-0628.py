...
COLUMNS = (
    ["observed_et", "observed_utc", "cohort_date", "cohort_dow", "age_days", "status"]
...
def main():
    ...
    for age in range(YOUNGEST_AGE, OLDEST_AGE + 1):
        ...
        base = f"{cohort:%Y/%m/%d}[{DATE_FIELD}]"
        row = {
            "observed_et":  f"{et:%Y-%m-%d %H:%M:%S}",
            "observed_utc": f"{now_utc:%Y-%m-%d %H:%M:%S}",
            "cohort_date":  f"{cohort:%Y-%m-%d}",
            "cohort_dow":   f"{cohort:%a}",
            "age_days":     age,
        }
        try:
            counts = {}
            for name, frag in MEASURES:
                counts[name] = _count(f"{base} AND {frag}")
                time.sleep(GAP_SECS)
            ...
            print(f"  {cohort:%Y-%m-%d} {cohort:%a} age {age:>2}  "
                  f"all_sb {counts['all_sb']:>7,}  ft {counts['fulltext']:>7,}{flag}")
        except Exception as e:
            # One bad cohort is a hole in that column, not a failed run. The
            # baseline is the exception: nothing can diff against it and it can
            # never be recaptured, so the run reports failure and can be re-fired.
            row["status"] = "NA"
            failed.append(str(cohort))
            if age == YOUNGEST_AGE:
                baseline_failed = True
            print(f"  {cohort:%Y-%m-%d} {cohort:%a} age {age:>2}  FAILED -- {e}")
        fresh.append(row)

    # A re-run replaces today's rows rather than doubling them, so firing the
    # workflow by hand after a failure is safe.
    ...
    print()
    print(f"  {len(fresh)} cohorts measured, {len(failed)} failed")
